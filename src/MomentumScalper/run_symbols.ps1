# Breakout EA (config B) on multiple symbols, real ticks, Mar20-Jul24 2026.
...
$symbols=@("EURUSD","GBPUSD","XAUUSD")   # XAUUSD included as the known baseline
...
foreach($sym in $symbols){
  $report="MScalp_sym_$sym"; $iniPath=Join-Path $iniDir "sym_$sym.ini"
  New-Ini $sym $report | Set-Content $iniPath -Encoding ASCII
  ...
  if(Test-Path $rep){
    ...
    $results+=[pscustomobject]@{ Symbol=$sym
  ...
  } else { $results+=[pscustomobject]@{ Symbol=$sym; Net="NO REPORT"; PF="";Trades="";Won="";MaxDD="" } }
}
