...
input bool            InpEnableAVS         = true;      // Scale lots: bigger with-trend, smaller counter-trend
input ENUM_TIMEFRAMES InpATRTimeframe      = PERIOD_M5; // ATR timeframe for AVS
input int             InpATRPeriod         = 14;        // ATR period
input double          InpBaseATR           = 150.0;     // Baseline ATR in POINTS (vol ratio = currentATR/base)
input double          InpCounterTrendMult  = 0.5;       // Counter-trend lot multiplier (dampen adverse adds)
input double          InpWithTrendMult     = 1.5;       // With-trend lot multiplier (expand aligned adds)
...
bool     m_halted = false;
uint     m_last_draw = 0;
int      m_hAVS = INVALID_HANDLE;                    // cached ATR handle for AVS (created once)
...
int OnInit()
  {
   ...
   //--- cache the AVS ATR handle once (avoids the per-tick handle leak in the draft)
   if(InpEnableAVS)
     { m_hAVS=iATR(m_sym,InpATRTimeframe,InpATRPeriod);
       if(m_hAVS==INVALID_HANDLE) Log("AVS: ATR handle failed, AVS will pass-through (x1.0)"); }
...
void OnDeinit(const int r){ if(m_hAVS!=INVALID_HANDLE) IndicatorRelease(m_hAVS); PanelDestroy(); ChartRedraw(0); }
...
//  ORDERS
//====================================================================
//--- Asymmetric Volatility Scaling: multiply the lot by a factor that is
//--- LARGER when the new position aligns with the recent bar's direction
//--- (with-trend) and SMALLER when it's counter-trend, times a volatility
//--- ratio (currentATR / baseline). Uses a CACHED ATR handle (created once
//--- in OnInit) - creating/releasing per call, as the draft did, leaks handles.
double GetAVSMultiplier(const ENUM_POSITION_TYPE side,const int level)
  {
   if(!InpEnableAVS || m_hAVS==INVALID_HANDLE) return 1.0;
   double atr[]; ArraySetAsSeries(atr,true);
   if(CopyBuffer(m_hAVS,0,0,1,atr)<=0) return 1.0;
   double curPts = atr[0]/m_point;
   double volRatio = (InpBaseATR>0)? curPts/InpBaseATR : 1.0;
   volRatio = MathMin(MathMax(volRatio,0.5),3.0);       // clamp 0.5x .. 3.0x

   double o1=iOpen (m_sym,InpATRTimeframe,1);
   double c1=iClose(m_sym,InpATRTimeframe,1);
   bool bull=(c1>o1);

   double mult=1.0;
   if(side==POSITION_TYPE_BUY)  mult = bull ? InpWithTrendMult : InpCounterTrendMult;
   else                         mult = !bull? InpWithTrendMult : InpCounterTrendMult;
   //--- dampen deep counter-trend levels further
   if(level>3 && !bull && side==POSITION_TYPE_BUY)  mult*=0.8;
   if(level>3 &&  bull && side==POSITION_TYPE_SELL) mult*=0.8;
...
bool OpenLevel(const ENUM_POSITION_TYPE side,const int level,const double lot)
  {
   //--- AVS: scale the requested lot asymmetrically by trend alignment + vol
   double avs=GetAVSMultiplier(side,level);
   double want=lot*avs;
   //--- clamp to broker volume min/max/step
   ...
   double use=MathMin(MathMax(want,vmin),vmax);
   if(vstep>0) use=MathFloor(use/vstep)*vstep;
   use=NormalizeDouble(use,2);

   string cmt=StringFormat("P%d",level); bool ok;
   if(side==POSITION_TYPE_BUY) ok=m_trade.Buy(use,m_sym,0,0,0,cmt);
   else                        ok=m_trade.Sell(use,m_sym,0,0,0,cmt);
   if(ok) m_lastOpen[side==POSITION_TYPE_BUY?0:1]=TimeCurrent();   // scalp throttle stamp
   Log(StringFormat("%s %s lot=%.2f (AVS x%.2f): %s rc=%u",side==POSITION_TYPE_BUY?"BUY":"SELL",cmt,use,avs,
